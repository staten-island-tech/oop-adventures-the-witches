...
while add_more_users == "Y":
    ...
    if user_request== "Townsfolk":
        ...
        w =Create_Towns_Log(Name,Occupation) 
        TownsLog.append(w)
        for i in TownsLog:
            print       
    ...
    class JournalCheck():
        def Journal_check():
            print("Welcome to the journal. What would you like to see today?")
            add_more_users = "Y"
            def check_tenure(status):
                if status.lower() == "y":
                    return True
            ...
            while add_more_users == "Y":
                user_request = input("Would You like to See anything else? Y/N")
